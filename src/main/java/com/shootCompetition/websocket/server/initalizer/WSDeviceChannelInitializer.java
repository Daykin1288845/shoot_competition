...
import com.shootCompetition.websocket.server.handler.DeviceConnectHandler;
import com.shootCompetition.websocket.server.handler.DeviceMessageHandler;
 ...
 */
public class WSDeviceChannelInitializer extends ChannelInitializer<SocketChannel> {
    ...
    private void initHandles(SocketChannel ch) {
        ...
        ch.pipeline().addLast("DeviceConnectHandler", new DeviceConnectHandler());
        ch.pipeline().addLast("WebSocketServerConnectHandler", new WebSocketServerConnectHandler());
        ch.pipeline().addLast("DeviceMessageHandler", new DeviceMessageHandler());
